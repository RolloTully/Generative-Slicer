...
def trussopt(width, height, st, sc, jc):
    poly = Polygon([(0, 0), (width, 0), (width, height), (0, height)]) # Defines a boundary
    convex = True if poly.convex_hull.area == poly.area else False #CHecks that the boundary is convex
    xv, yv = np.meshgrid(range(width+1), range(height+1)) # Creates a Mesh the size of the grid
    pts = [Point(xv.flat[i], yv.flat[i]) for i in range(xv.size)] # Converts the mesh grid in to a set of caertersian poijtns
    Nd = np.array([[pt.x, pt.y] for pt in pts if poly.intersects(pt)]) # FIlters all points to be within the Boundary
    print("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@''")
    print(Nd)
...
if __name__ =='__main__':
    trussopt(width = 50, height = 25, st = 1, sc =1, jc = 1)
